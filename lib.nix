_: _: let
  fetchTree = info:
    if info.type == "github" then
      { outPath =
        ...
        lastModifiedDate = _formatSecondsSinceEpoch info.lastModified;
  ...
  callFlake4 = flakeSrc: lockFileInputs:
    let
      ...
      inputs = builtins.mapAttrs (n: v:
        ...
        else fetchTree (v.locked // v.info)) lockFileInputs;
  ...
  mkAllNodes = rootSrc: lockFile: let
  ...
  mkDefaultNix = system: flake:
    (builtins.removeAttrs flake ["__functor"])
      // (
        if flake ? defaultPackage.${system}
        then { default = flake.defaultPackage.${system}; }
        else {}
      )
      // (
        if flake ? packages.${system}.default
        then { default = flake.packages.${system}.default; } else {}
      );

  mkShellNix = system: flake:
    (mkDefaultNix system flake)
      // (
        if flake ? devShell.${system}
        then { default = flake.devShell.${system}; }
        else {}
      )
      // (
        if flake ? devShells.${system}.default
        then { default = flake.devShells.${system}.default; }
        else {}
      );
in {
  fc = {
